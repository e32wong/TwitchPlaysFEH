...
public class GameStateFEH2 extends GameBase {
    ...
    public void securityCheck(String userName) {
        ...
        try {
            ...
            String[] gameDataTerms = {"Inherit this selection?", "Return selected character",
                "Combat Manual.", "to obtain Hero Feathers", "Give target ally", "Level and stats", "will return to initial",
                "you want to purchase", "Select the number of Orbs", "CA$", "Purchasey",
                "Europe/Australia)", "high scores on leaderboards",
                "data for Fire Emblem Heroes", "You will no longer be able", "access this data on any device",
                "Raise stats of target ally", 
                "You can use an Aether Resort Pass", "you get your daily R&R Affinity", // aether resort pass buying
                "characters or fewer", "change your name later", "cannot use some special symbols", // name change
                "Orb Balance", // customer support
                "What kind of application is Fire Emblem Heroes?", // FAQ
                "choose the Lite Data version", "data quality to Lite Data Version", // data quality
            };

            String[] settingsDataTerms = {
                "Combat Animations", "Support Animations", "Auto-Favorite", "Battle Movement", // Settings
                "My Achi... Google Play", // Achievments
            };

            String detectedText = ImageAnalysis.getTextPosition(gameDataTerms, 1080, 1920 - 200, 0, 0);

            {
                ...
                if (foundTerms.size() > 0) {
					...
					String message = "@" + userName + " We had detected attempts to perform skill inheritance/" +
						"sending home/unlock potential/purchase orbs" +
						" and will reboot the game!" +
						" The reason is because skill inheritance will delete all the units easily." +
						" Sorry if it is a false positive, the " +
						"bot will resume after rebooting (it takes 2 minutes to fully reboot) :)";

                    connection.sendText(message);
                    if (!userName.equals("console")) {
                        processReboot();
                    } else {
                        System.out.println("Ignored reboot");
                    }
                }
            }
            ...
            {
                // settings terms
                ArrayList<String> foundTerms = HelpTools.containsText(detectedText, settingsDataTerms);
                if (foundTerms.size() > 0) {

					String message = "@" + userName + " The action had been blocked!";

                    connection.sendText(message);
                    if (!userName.equals("console")) {
                        device.sendKeyEvent(4);
                    } else {
                        System.out.println("Ignored back button");
                    }
                }
            }
